...
class Template extends template.RecursiveTemplate {
    constructor(options) {
        ...
        this.buildFlattenView = (level = this.flattenViewLevel) => {
            ...
            const template = `<view s-if="{{item.nn==='view'&&(item.st||item.cl)}}" id="{{item.uid}}" ${this.buildFlattenNodeAttributes('view')}>
...
<block s-else>
  <template is="{{xs.e(0)}}" data="{{{i:item}}}" />
        ...
        this.buildFlattenCover = (level = this.flattenCoverLevel) => {
            ...
            const template = `<cover-view s-if="{{item.nn==='cover-view'}}" id="{{item.uid}}" ${this.buildFlattenNodeAttributes('cover-view')}>
  <block s-for="{{item.cn}}" s-key="uid">
    ${shared.indent(child, 4)}
  </block>
</cover-view>
  ...
  <template is="{{xs.e(0)}}" data="{{{i:item}}}" />
        ...
        this.modifyLoopBody = (child, nodeName) => {
            switch (nodeName) {
                ...
                case 'video': {
                    const body = `<ad s-if={{item.nn==='ad'}} id="{{item.uid}}" ${this.buildFlattenNodeAttributes('ad')}></ad>
<block s-else>
  ${shared.indent(this.buildFlattenCover(), 2)}
        ...
        this.flattenCoverLevel = (_b = options === null || options === void 0 ? void 0 : options.flattenViewLevel) !== null && _b !== void 0 ? _b : 3;
    }
